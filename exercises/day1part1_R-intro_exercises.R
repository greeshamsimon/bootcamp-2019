...
#mean(vec) does not work cause of NA
mean(vec, na.rm = TRUE)

# Count the number of missing values in vec

is.na(vec)
sum(is.na(vec)) #spits how many NA values are there
mean(is.na(vec)) #spits the average, i.e. % of NA values
#### Factors ####
...
#you can use tab to parse through directory
...
view(gapminder07)
secondgapminder07 <- subset(gapminder,subset= year == 2007)
...
aggregate(lifeExp ~ year + continent, data = gapminder, FUN=mean)
...
t2 <-t.test(gapminder07$gdpPercap~gapminder07$lifeExp_over70)
t2
#### Statistics, part 2 ####
...
#### Save R objects ####
#save.image() useful for saving environment during personal worklflow
...
hist(gapminder07$lifeExp, main = "Distribution of Life Expectancy across Countries in 2007",
     xlab = "Life Expectancy", ylab = "frequency", breaks =5 )
...
plot(gapminder07$lifeExp ~ gapminder07$gdpPercap, main = "Life Expectancy vs. GDP Per Capita", 
...
newlm= lm(gapminder07$lifeExp ~ gapminder07$gdpPercap)
abline (newlm$coefficients[1], b = newlm$coefficients[2])
        )